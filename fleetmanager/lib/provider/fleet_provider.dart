...
class FleetProvider with ChangeNotifier {
  ...
  /// --- LOGICA PRENOTAZIONI ---
  Future<void> creaPrenotazione(Utente driver, Veicolo veicolo, DateTime inizio, DateTime fine) async {
    ...
    notifyListeners();
    try {
      if (!_isSlotDisponibile(targa: veicolo.targa, idUtente: driver.idUtente, inizio: inizio, fine: fine)) {
      ...
      for (var idManager in _tuttiManagerIds) {
        await _notificaService.notificaRichiestaPrenotazione(
            idManager, "${driver.nome} ${driver.cognome}", veicolo.targa, inizio, fine);
      }
      await inizializzaDati();
    } finally {
      _isLoading = false;
  ...
  Future<void> confermaPrenotazione(int id) async {
    try {
      ...
      final p = _prenotazioni.firstWhere((element) => element.idPrenotazione == id);
    ...
    try {
      ...
      if (_utenteLoggato?.ruoloUtente == RuoloUtente.manager) {
        ...
        for (var idMan in _tuttiManagerIds) {
          await _notificaService.notificaAnnullamentoPrenotazioneDaDriver(idMan, p.idUtente, p.targa);
        }
      }
      await inizializzaDati();
    } catch (e) { debugPrint(e.toString()); }
  ...
  Future<void> programmareManutenzione(Veicolo veicolo, DateTime inizio, TipoManutenzione tipo, String descrizione, {required String luogo}) async {
    try {
      await _manutenzioneService.registraIntervento(Manutenzione(idManutenzione: 0, data: inizio, tipoManutenzione: tipo, descrizione: descrizione, targa: veicolo.targa, luogo: luogo));
      if (tipo == TipoManutenzione.straordinaria || inizio.isBefore(DateTime.now().add(const Duration(minutes: 10)))) {
        await _veicoloService.updateStatoVeicolo(veicolo.targa, StatoVeicolo.inManutenzione);
        for (var idMan in _tuttiManagerIds) { await _notificaService.notificaInterventoStraordinario(idMan, veicolo.targa); }
      }
      await inizializzaDati();
    } catch (e) { rethrow; }
  }

  Future<void> chiudiManutenzione(int idManutenzione, String targa, {int? nuoviKm}) async {
    try {
      int kmFinali = nuoviKm ?? _veicoli.firstWhere((v) => v.targa == targa).km;
      await _manutenzioneService.chiudiIntervento(idManutenzione, kmFinali, targa);
      await inizializzaDati();
    } catch (e) { rethrow; }
  }

  bool isVeicoloDisponibile(String targa, DateTime inizioReq, DateTime fineReq) => _isSlotDisponibile(targa: targa, idUtente: 0, inizio: inizioReq, fine: fineReq);

  Future<void> aggiungiNuovoUtente({required String email, required String passwordScelta, required String nome, required String cognome, required String ruolo, String? patente}) async {
    try {
      final authRes = await Supabase.instance.client.auth.signUp(email: email.trim(), password: passwordScelta);
      if (authRes.user != null) {
        await Supabase.instance.client.from('utenti').insert({'email': email.trim().toLowerCase(), 'nome': nome.trim(), 'cognome': cognome.trim(), 'ruolo': ruolo, 'patente': patente ?? 'Da inserire'});
        await inizializzaDati();
      }
    } catch (e) { rethrow; }
  }

  Future<void> aggiungiNuovoVeicolo({required String targa, required String marca, required String modello, required String tipo, required String anno, String? kmAttuali}) async {
    try {
      await Supabase.instance.client.from('veicoli').insert({'targa': targa.trim().toUpperCase(), 'marca': marca.trim(), 'modello': modello.trim(), 'tipo': tipo, 'stato': 'disponibile', 'km': int.tryParse(kmAttuali ?? '0') ?? 0, 'anno_immatricolazione': int.tryParse(anno) ?? DateTime.now().year});
      await inizializzaDati();
    } catch (e) { rethrow; }
  }

  Future<void> eliminaUtente(int id) async { await _authService.eliminaUtente(id); await inizializzaDati(); }
  Future<void> eliminaVeicolo(String targa) async { await Supabase.instance.client.from('veicoli').delete().eq('targa', targa); await inizializzaDati(); }

  Future<void> modificaPrenotazione(int idPrenotazione, DateTime nuovoInizio, DateTime nuovaFine) async {
    try {
      final p = _prenotazioni.firstWhere((element) => element.idPrenotazione == idPrenotazione);
      final driver = _utenti.firstWhere((u) => u.idUtente == p.idUtente);
      if (!_isSlotDisponibile(targa: p.targa, idUtente: p.idUtente, inizio: nuovoInizio, fine: nuovaFine, idDaEscludere: idPrenotazione)) { throw Exception('Sovrapposizione.'); }
      await Supabase.instance.client.from('prenotazioni').update({'data_inizio': nuovoInizio.toIso8601String(), 'data_fine': nuovaFine.toIso8601String(), 'stato': 'richiesta'}).eq('id_prenotazione', idPrenotazione);
      for (var idMan in _tuttiManagerIds) { await _notificaService.notificaRichiestaPrenotazione(idMan, "${driver.nome} ${driver.cognome}", p.targa, nuovoInizio, nuovaFine); }
      await inizializzaDati();
    } catch (e) { rethrow; }
  }

  bool _isSlotDisponibile({required String targa, required int idUtente, required DateTime inizio, required DateTime fine, int? idDaEscludere}) {
    final veicoloInOfficina = _manutenzioni.any((m) => m.targa == targa && m.oraFine == null && inizio.isBefore(m.data.add(const Duration(hours: 8))) && fine.isAfter(m.data));
    ...
    return !_prenotazioni.any((p) {
      ...
      final haSovrapposizioneOraria = inizio.isBefore(p.dataFine) && fine.isAfter(p.dataInizio);
      return haSovrapposizioneOraria && (p.targa == targa || (idUtente != 0 && p.idUtente == idUtente));
    });
  }
}